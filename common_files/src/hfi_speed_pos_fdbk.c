...
void HFI_update(HFI_Handle_t * pHandle){
	if(pHandle->d_cnt > pHandle->samples_avg){
		int32_t ind = pHandle->d_out / pHandle->d_cnt;
		pHandle->d_out=0;
		pHandle->d_cnt=0;
		pHandle->inductance_now = ind;

		pHandle->hfi_angle -= ind/2;
		...
		pHandle->speed_avg = (diff + (16 - 1) * pHandle->speed_avg) >> 4;
		...
		pHandle->Vinject.alpha = ((int32_t)pHandle->hfi_voltage * (int32_t)pHandle->trig.hCos) >> 15;
		pHandle->Vinject.beta = ((int32_t)pHandle->hfi_voltage * (int32_t)pHandle->trig.hSin) >> 15;
